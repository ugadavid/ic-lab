...
function renderAiModels(models = aiModelsCache) {
  ...
  aiModelsBox.innerHTML = filtered.map((model) => `
    <form class="ai-model-form row ai-model-card" data-model-id="${model.id}">
      <strong>${model.title}</strong>
      <p>${model.shortDescription || "Description a completer."}</p>
      <p>${model.providerId} - ${model.providerModelId} - ${model.family || "n/a"} - ${model.modality || "n/a"} - ${model.status}</p>
      <p>Capacites: ${(model.capabilities || []).join(", ") || "n/a"} - recommandation ${recommendationLabel(model.icLabRecommendation)} - cout ${model.costLevel || "Prix non renseigne"}</p>
      <p>Prix: ${priceLabel(model)} - source ${model.pricing?.source || "a completer"} - verifie ${model.pricing?.lastCheckedAt || "n/a"}</p>
      <p>Usage: ${model.recommendedUse || "A documenter."}</p>
      <p>Teacher ${boolLabel(model.allowedForTeachers)} - Student ${boolLabel(model.allowedForStudents)} - Runtime ${boolLabel(model.allowedForRuntime)} - lastSeenAt ${model.lastSeenAt || "n/a"}</p>
      <label>Titre <input name="title" value="${model.title || ""}"></label>
      <label>Description courte <textarea name="shortDescription">${model.shortDescription || ""}</textarea></label>
      <label>Statut <input name="status" value="${model.status || ""}"></label>
      <label>Cout <input name="costLevel" value="${model.costLevel || ""}"></label>
      <label>Recommandation IC-Lab <input name="icLabRecommendation" value="${model.icLabRecommendation || ""}"></label>
      <label>Prix input <input name="inputPriceUsd" type="number" step="0.000001" value="${model.pricing?.inputUsd ?? ""}"></label>
      <label>Prix input cache <input name="cachedInputPriceUsd" type="number" step="0.000001" value="${model.pricing?.cachedInputUsd ?? ""}"></label>
      <label>Prix output <input name="outputPriceUsd" type="number" step="0.000001" value="${model.pricing?.outputUsd ?? ""}"></label>
      <label>Unite prix <input name="pricingUnit" value="${model.pricing?.unit || ""}"></label>
      <label>Source prix <input name="pricingSource" value="${model.pricing?.source || ""}"></label>
      <label>Prix verifie le <input name="pricingLastCheckedAt" value="${model.pricing?.lastCheckedAt || ""}" placeholder="YYYY-MM-DD"></label>
