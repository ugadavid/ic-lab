    ...
    return `
      <article class="card">
        ...
        <ul class="assignments">${assignments.map((item) => {
          ...
          return `<li>${item.prototypeId} / ${assignmentLabel(item)} (${item.activitySource})${launchLink}<br><small>${ownershipMeta(item)}</small>${debugMeta({ ownerId: item.ownership?.ownerId, institutionId: item.ownership?.institutionId, visibility: item.ownership?.visibility || item.visibility, provenance: item.ownership?.provenance })}</li>`;
