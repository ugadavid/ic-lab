...
const allowedCharacters = new Set(["Clara", "Marco", "Ana", "Malik", "Lucia", "Lucía", "Andrei", "Laia"]);
const allowedLanguages = new Set(["es-ES", "it-IT", "pt-BR", "fr-FR", "es-AR", "ro-RO", "ca-ES"]);
const allowedScenarios = new Set([
  "globalUnderstanding",
  "selectiveUnderstanding",
  "detailedUnderstanding",
  "intercomprehensionStrategies",
  "climateObservations",
  "personalTestimony"
]);
...
function cleanOpenAiError(error) {
  ...
  const next = runtimeError("OPENAI_RUNTIME_ERROR", error.message || "Erreur OpenAI.", error.status || 502);
  return next;
}
...
function validateDraftInput(input) {
  ...
  return { characterId, targetLanguage, scenarioId, climateTheme, maxWords };
}

function draftPrompt({ characterId, targetLanguage, scenarioId, climateTheme, maxWords }) {
  return [
...
async function generateIcAgentDraft({ modelCatalogId, characterId, targetLanguage, scenarioId, climateTheme, maxWords, catalogModels = [] }) {
  try {
    const model = validateOpenAiCatalogModel(modelCatalogId, catalogModels);
    const input = validateDraftInput({ characterId, targetLanguage, scenarioId, climateTheme, maxWords });
    const started = Date.now();
    const response = await client().responses.create({
      model: model.providerModelId,
