...
function renderLabResult(title, payload, kind = "ok") {
  ...
  const text = payload?.text ? `<pre>${esc(payload.text)}</pre>` : "";
...
function setLabBusy(isBusy, label = "") {
  [checkOpenAiRuntimeButton, smokeOpenAiButton, ...document.querySelectorAll(".draft-button")].forEach((button) => {
