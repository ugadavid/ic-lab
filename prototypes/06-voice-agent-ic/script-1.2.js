...
let proto06ServerConnected = false;
let proto06ServerEndpoint = "";
let proto06ServerError = "";
const hubLaunchState = {
  params: {},
  activityLoadedFromUrl: false,
  error: "",
  aiConfig: null,
  aiConfigError: "",
  runtimeWarnings: [],
...
function readHubLaunchParams() {
  const params = new URLSearchParams(window.location.search);
  const activityId = params.get("activityId")?.trim() || "";
  const activitySource = params.get("activitySource")?.trim() || "";
  ...
  const aiConfigId = params.get("aiConfigId")?.trim() || "";
  hubLaunchState.params = { activityId, activitySource, courseId, assignmentId, runId, launchToken, aiConfigId };
  return hubLaunchState.params;
}

function applyHubLaunchParams() {
  const { activityId, activitySource } = readHubLaunchParams();
  if (!activityId) {
    return;
  }

  const source = activitySource || "server";
  localStorage.setItem("proto06SelectedActivity", activityId);
  localStorage.setItem("proto06SelectedActivitySource", source);
  hubLaunchState.activityLoadedFromUrl = true;
}

applyHubLaunchParams();

function isProto06DebugEnabled() {
  return localStorage.getItem("proto06Debug") === "1";
}

function rememberHubRunEvent(collection, item) {
...
function sendHubRunEvent(type, payload = {}) {
  const { runId, launchToken } = hubLaunchState.params;
  if (!runId || !launchToken) {
    return Promise.resolve(false);
  }

  const stamp = new Date().toISOString();
  rememberHubRunEvent(hubLaunchState.runEvents, { type, at: stamp });
  renderDebugPanel();

  return fetch(`http://127.0.0.1:8790/api/runs/${encodeURIComponent(runId)}/events`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ launchToken, type, payload })
  })
    .then((response) => {
      if (!response.ok) {
        throw new Error(`Hub run event ${response.status}`);
      }
      return response.json().catch(() => ({}));
    })
    .catch((error) => {
      const failure = { type, at: new Date().toISOString(), message: error.message };
      rememberHubRunEvent(hubLaunchState.runEventErrors, failure);
      if (isProto06DebugEnabled()) {
        console.warn("[hub-run-event]", failure);
      }
      renderDebugPanel();
      return false;
    });
...
const scriptedAgentRuntime = {
  id: "scripted",
  label: "scenarise securise",
  runtimeEnabled: true,
  canGenerateText: false,
  canGenerateAudio: false,
  resolveTurn(context = {}) {
    const participant = context.participant || {};
    const feedbackResult = context.feedbackResult || null;

    if (context.kind === "feedback" && feedbackResult) {
      return {
        text: feedbackResult.response,
        language: "fr",
        speechLang: languageCatalog.fr.speechLang,
        source: "scripted",
        runtimeId: this.id,
        generated: false
      };
    ...
    return {
      ...
      language: participant.languageLabel || "",
      speechLang: participant.lang || languageCatalog.fr.speechLang,
      source: "scripted",
      runtimeId: this.id,
      generated: false
    };
  ...
  describeRuntime(aiConfig) {
    return {
      assignedMode: aiConfig?.mode || "scripted",
      selectedRuntime: this.id,
      label: this.label,
      fallback: false,
      generated: false,
...
const disabledFutureRuntime = {
  id: "disabled_future_runtime",
  label: "runtime futur desactive",
  runtimeEnabled: false,
  canGenerateText: false,
  canGenerateAudio: false,
  resolveTurn(context = {}) {
    return scriptedAgentRuntime.resolveTurn(context);
  },
  describeRuntime(aiConfig) {
    return {
      assignedMode: aiConfig?.mode || "non renseigne",
      selectedRuntime: "scripted",
      label: scriptedAgentRuntime.label,
      fallback: true,
      generated: false,
      runtimeEnabled: false,
      warnings: ["Le mode IA selectionne est preparatoire. Prototype 06 utilise le mode scenarise securise."]
    };
  }
};
...
const agentRuntimeRegistry = {
  ...
  llm_text_browser_voice: disabledFutureRuntime,
  llm_text_api_voice: disabledFutureRuntime,
  realtime_voice_agent: disabledFutureRuntime,
  multi_agent_experimental: disabledFutureRuntime
};

const runtimeModeAliases = {
  "llm-text": "llm_text_browser_voice",
  "llm-text-api-voice": "llm_text_api_voice",
  "realtime-voice": "realtime_voice_agent",
  "multi-agent": "multi_agent_experimental"
};

function normalizeRuntimeMode(mode) {
  const rawMode = String(mode || "scripted").trim();
  return runtimeModeAliases[rawMode] || rawMode.replace(/-/g, "_") || "scripted";
}

function selectAgentRuntime(aiConfig) {
  const assignedMode = normalizeRuntimeMode(aiConfig?.mode);
  const assignedRuntime = agentRuntimeRegistry[assignedMode] || disabledFutureRuntime;
  const warnings = [];
  let fallback = false;
  ...
  if (aiConfig.runtimeEnabled !== true) {
    fallback = assignedMode !== "scripted";
    warnings.push("runtimeEnabled=false : aucune IA reelle n'est activee.");
  }

  if (aiConfig.status !== "active") {
    fallback = true;
    warnings.push(`Statut ${aiConfig.status || "inconnu"} : configuration preparatoire.`);
  }

  if (assignedRuntime !== scriptedAgentRuntime) {
    fallback = true;
    warnings.push("Le mode IA selectionne est preparatoire. Prototype 06 utilise le mode scenarise securise.");
  }
...
function updateActiveAgentRuntime(aiConfig) {
  const selection = selectAgentRuntime(aiConfig);
  activeAgentRuntime = selection.runtime;
  activeRuntimeState = selection;
  if (selection.warnings.length) {
    selection.warnings.forEach((message) => rememberHubRunEvent(hubLaunchState.runtimeWarnings, {
      type: "runtime",
...
function emitRuntimeSelectionEvents(selection, aiConfig) {
  if (!aiConfig) return;
  sendHubRunEvent("ai_config_resolved", {
    aiConfigId: aiConfig.id || hubLaunchState.params.aiConfigId || "",
    status: aiConfig.status || "",
    mode: aiConfig.mode || "",
    runtimeEnabled: aiConfig.runtimeEnabled === true
  });
  sendHubRunEvent("agent_runtime_selected", {
    assignedMode: selection.assignedMode,
...
function resolveHubAiConfig() {
  const { runId, launchToken, aiConfigId } = hubLaunchState.params;
  if (!runId || !launchToken || !aiConfigId) {
    updateActiveAgentRuntime(null);
    return Promise.resolve(null);
  }

  return fetch(`http://127.0.0.1:8790/api/runs/${encodeURIComponent(runId)}/ai-config`, {
    headers: { "x-launch-token": launchToken }
  })
    .then((response) => {
      if (!response.ok) {
        throw new Error(`Hub AI config ${response.status}`);
      }
      return response.json();
    })
    .then((body) => {
      hubLaunchState.aiConfig = body.aiConfig || null;
      hubLaunchState.aiConfigError = "";
      const selection = updateActiveAgentRuntime(hubLaunchState.aiConfig);
      emitRuntimeSelectionEvents(selection, hubLaunchState.aiConfig);
      return hubLaunchState.aiConfig;
    })
    .catch((error) => {
      hubLaunchState.aiConfig = null;
      hubLaunchState.aiConfigError = error.message;
      updateActiveAgentRuntime(null);
      if (isProto06DebugEnabled()) {
        console.warn("[hub-ai-config]", error);
      }
      renderActivitySummary();
      renderDebugPanel();
      return null;
...
function fetchServerActivitySync(activityId) {
  proto06ServerEndpoint = `${SERVER_API_BASE}/activities/${encodeURIComponent(activityId)}`;
  try {
    const request = new XMLHttpRequest();
    request.open("GET", proto06ServerEndpoint, false);
    request.send();
    if (request.status < 200 || request.status >= 300) {
      throw new Error(`API serveur ${request.status}`);
    }
    proto06ServerConnected = true;
    proto06ServerError = "";
    return JSON.parse(request.responseText);
  } catch (error) {
    proto06ServerConnected = false;
    proto06ServerError = error.message;
    hubLaunchState.error = error.message;
    return null;
  }
}
...
function getStoredActivity() {
  const stored = localStorage.getItem("proto06SelectedActivity");
  const source = localStorage.getItem("proto06SelectedActivitySource") || "catalog";

  if (stored && source === "catalog" && activityCatalog[stored]) {
    return activityCatalog[stored];
  }

  if (stored && source === "local") {
    try {
      const customActivities = JSON.parse(localStorage.getItem("proto06CustomActivities"));
      if (Array.isArray(customActivities)) {
        return customActivities.find((activity) => activity.id === stored) || null;
      }
    } catch {
      return null;
    }
  ...
  if (stored && source === "server") {
    return fetchServerActivitySync(stored);
  }

  return null;
}

function getStoredActivitySource() {
  return localStorage.getItem("proto06SelectedActivitySource") || "catalog";
}

function persistActivitySelectionDetails(activity) {
  if (!activity) {
    return;
  }

  if (Array.isArray(activity.characterIds)) {
    localStorage.setItem("proto06SelectedCharacters", JSON.stringify(activity.characterIds));
  }

  if (activity.scenarioId) {
    localStorage.setItem("proto06SelectedPedagogicalScenario", activity.scenarioId);
  }
}
...
const selectedActivity = getStoredActivity();
const selectedActivitySource = selectedActivity ? getStoredActivitySource() : getStoredActivitySource();
persistActivitySelectionDetails(selectedActivity);
const selectedCharacterIds = selectedActivity
  ? selectedActivity.characterIds
  : getStoredSelection();
const selectedPedagogicalScenarioId = selectedActivity
  ? selectedActivity.scenarioId
  : getStoredPedagogicalScenarioId();
const stagedScenario = buildScenarioFromPedagogicalSelection(
  selectedCharacterIds,
  selectedPedagogicalScenarioId
);
stagedScenario.activity = selectedActivity;
stagedScenario.activitySource = selectedActivitySource;
...
const teacherActivityCard = document.querySelector("#teacherActivityCard");
const activityTitle = document.querySelector("#activityTitle");
const activityGoal = document.querySelector("#activityGoal");
...
let debugPanel = null;
let hubActivityCompletedSent = false;
let hubLoadErrorSent = false;
let activeAgentRuntime = scriptedAgentRuntime;
let activeRuntimeState = selectAgentRuntime(null);
let lastRuntimeTurn = null;

function sendHubActivityCompletedOnce(reason) {
  if (hubActivityCompletedSent) {
    return;
  }
  hubActivityCompletedSent = true;
  sendHubRunEvent("activity_completed", {
    activityId: stagedScenario.activity?.id || hubLaunchState.params.activityId || "",
    reason
  });
}
...
function getDebugState() {
  ...
  return {
    activity: stagedScenario.activity,
    activitySource: stagedScenario.activitySource,
...
function ensureDebugPanel() {
  ...
  debugPanel.innerHTML = `
    <div class="debug-header">
      ...
      <div class="debug-actions">
        <button class="debug-reset-meeting" type="button">Reset rencontre</button>
        <button class="debug-disable" type="button">Désactiver debug</button>
      </div>
    </div>
  ...
  debugPanel
    .querySelector(".debug-reset-meeting")
    .addEventListener("click", () => resetProto06State({ keepDebug: true }));
  debugPanel
    .querySelector(".debug-disable")
...
function formatDebugValue(value) {
  if (Array.isArray(value)) {
    return value.length ? value.map(escapeHtml).join(" | ") : "aucun";
  }
...
function aiConfigSummaryHtml() {
  const config = hubLaunchState.aiConfig;
  const requestedId = hubLaunchState.params.aiConfigId || "non renseigne";

  if (!config) {
    const warning = hubLaunchState.aiConfigError
      ? ` Configuration non resolue depuis le hub : ${escapeHtml(hubLaunchState.aiConfigError)}.`
      : "";
    return `<div class="ai-config-note ai-config-note--warning"><strong>Mode IA / voix :</strong> ${escapeHtml(requestedId)}.${warning} <strong>Runtime utilise :</strong> ${escapeHtml(activeAgentRuntime.label)}. <strong>IA reelle :</strong> non activee.</div>`;
  }

  const status = config.status || "non renseigne";
  const preparatoryStatuses = new Set(["planned", "prototype", "experimental", "research"]);
  const warnings = Array.isArray(config.warnings) ? config.warnings : [];
  const statusWarning = preparatoryStatuses.has(status)
    ? `<p class="ai-config-warning">Mode preparatoire : cette configuration decrit une intention future, sans activation IA dans cette version.</p>`
    : "";
  const warningsHtml = warnings.length
    ? `<p class="ai-config-warning">${warnings.map(escapeHtml).join(" ")}</p>`
    : "";
  const runtimeWarningsHtml = activeRuntimeState.warnings.length
    ? `<p class="ai-config-warning">${activeRuntimeState.warnings.map(escapeHtml).join(" ")}</p>`
    : "";

  return `
    <div class="ai-config-note">
      <strong>Mode IA / voix :</strong> ${escapeHtml(config.title || requestedId)}
      (${escapeHtml(config.mode || "mode inconnu")} / ${escapeHtml(config.voiceMode || "voix inconnue")})<br>
      <strong>Provider :</strong> ${escapeHtml(config.provider || "none")} &middot;
      <strong>Voix :</strong> ${escapeHtml(config.voiceProvider || "browser")} &middot;
      <strong>Statut :</strong> ${escapeHtml(status)} &middot;
      <strong>Cout :</strong> ${escapeHtml(config.estimatedCostLevel || "non renseigne")} &middot;
      <strong>Duree max :</strong> ${escapeHtml(config.maxDurationSeconds || "non renseignee")} s<br>
      <strong>Role pedagogique :</strong> ${escapeHtml(config.pedagogicalRole || "non renseigne")} &middot;
      <strong>IA reelle :</strong> non activee<br>
      <strong>Runtime utilise :</strong> ${escapeHtml(activeAgentRuntime.label)}<br>
      <strong>Config assignee :</strong> ${escapeHtml(config.title || requestedId)}<br>
      <strong>Fallback :</strong> ${activeRuntimeState.fallback ? "oui" : "non"}
      ${statusWarning}
      ${warningsHtml}
      ${runtimeWarningsHtml}
    </div>
...
function renderDebugPanel() {
  ...
  const aiConfig = hubLaunchState.aiConfig || {};

  content.innerHTML = `
    <section class="debug-meta" aria-label="Contexte technique et pédagogique">
      <dl>
        <dt>Activité ID</dt>
        <dd>${state.activity?.id || "aucune"}</dd>
        <dt>Paramètres URL</dt>
        <dd>${JSON.stringify(hubLaunchState.params)}</dd>
        <dt>Activité chargée depuis URL</dt>
        <dd>${hubLaunchState.activityLoadedFromUrl ? "oui" : "non"}</dd>
        <dt>Erreur URL / hub</dt>
        <dd>${hubLaunchState.error || "aucune"}</dd>
        <dt>Etat serveur</dt>
        <dd>${proto06ServerConnected ? "connecte" : "indisponible"}</dd>
        <dt>Endpoint serveur</dt>
        <dd>${proto06ServerEndpoint || "n/a"}</dd>
        <dt>Erreur API</dt>
        <dd>${proto06ServerError || "aucune"}</dd>
        <dt>Run hub</dt>
        <dd>${hubLaunchState.params.runId || "aucun"}</dd>
        <dt>AIConfig ID URL</dt>
        <dd>${formatDebugValue(hubLaunchState.params.aiConfigId || "aucun")}</dd>
        <dt>AIConfig titre</dt>
        <dd>${formatDebugValue(aiConfig.title)}</dd>
        <dt>AIConfig mode</dt>
        <dd>${formatDebugValue(aiConfig.mode)}</dd>
        <dt>AIConfig provider</dt>
        <dd>${formatDebugValue(aiConfig.provider)}</dd>
        <dt>AIConfig modele</dt>
        <dd>${formatDebugValue(aiConfig.modelId)}</dd>
        <dt>AIConfig voix</dt>
        <dd>${formatDebugValue(aiConfig.voiceMode)} / ${formatDebugValue(aiConfig.voiceProvider)}</dd>
        <dt>AIConfig cout</dt>
        <dd>${formatDebugValue(aiConfig.estimatedCostLevel)}</dd>
        <dt>AIConfig duree max</dt>
        <dd>${formatDebugValue(aiConfig.maxDurationSeconds)}</dd>
        <dt>AIConfig role pedagogique</dt>
        <dd>${formatDebugValue(aiConfig.pedagogicalRole)}</dd>
        <dt>AIConfig statut</dt>
        <dd>${formatDebugValue(aiConfig.status)}</dd>
        <dt>AIConfig avertissements</dt>
        <dd>${formatDebugValue(aiConfig.warnings)}</dd>
        <dt>AIConfig runtime</dt>
        <dd>${aiConfig.runtimeEnabled === false ? "runtimeEnabled=false - IA reelle non activee" : "non resolue - IA reelle non activee"}</dd>
        <dt>Runtime selectionne</dt>
        <dd>${formatDebugValue(activeRuntimeState.selectedRuntime)}</dd>
        <dt>Runtime enabled</dt>
        <dd>${activeRuntimeState.runtimeEnabled ? "true" : "false"}</dd>
        <dt>Runtime fallback</dt>
        <dd>${activeRuntimeState.fallback ? "oui" : "non"}</dd>
        <dt>Dernier tour source</dt>
        <dd>${formatDebugValue(lastRuntimeTurn?.source)}</dd>
        <dt>Dernier tour generated</dt>
        <dd>${lastRuntimeTurn ? String(Boolean(lastRuntimeTurn.generated)) : "n/a"}</dd>
        <dt>Dernier tour runtimeId</dt>
        <dd>${formatDebugValue(lastRuntimeTurn?.runtimeId)}</dd>
        <dt>Warnings runtime</dt>
        <dd>${formatDebugValue(activeRuntimeState.warnings)}</dd>
        <dt>Erreur AIConfig</dt>
        <dd>${formatDebugValue(hubLaunchState.aiConfigError || "aucune")}</dd>
        <dt>Launch token</dt>
        <dd>${hubLaunchState.params.launchToken ? "present" : "absent"}</dd>
        <dt>Evenements hub envoyes</dt>
        <dd>${hubLaunchState.runEvents.map((event) => `${event.type} @ ${event.at}`).join(" | ") || "aucun"}</dd>
        <dt>Erreurs evenements hub</dt>
        <dd>${hubLaunchState.runEventErrors.map((event) => `${event.type}: ${event.message}`).join(" | ") || "aucune"}</dd>
        <dt>Source activité</dt>
        <dd>${state.activitySource || "n/a"}</dd>
        <dt>Activité active</dt>
        <dd>${state.activity?.title || "Composition manuelle"}</dd>
        <dt>Objectif pédagogique</dt>
        <dd>${state.activity?.pedagogicalGoal || "n/a"}</dd>
        <dt>Notes enseignantes</dt>
        <dd>${state.activity?.teacherNotes || "n/a"}</dd>
        <dt>Activité tags/version</dt>
        <dd>${state.activity ? `${state.activity.tags.join(", ")} · ${state.activity.version} · ${state.activity.updatedAt}` : "n/a"}</dd>
        <dt>JSON activité</dt>
        <dd>${state.activity ? JSON.stringify(state.activity) : "n/a"}</dd>
        <dt>Scénario ID</dt>
        <dd>${state.pedagogicalScenario?.id || "non défini"}</dd>
        <dt>Titre du scénario</dt>
        <dd>${state.pedagogicalScenario?.title || "non défini"}</dd>
        ...
        <dt>Fallback utilisé</dt>
        <dd>${participant ? (participant.fallbackUsed ? "oui" : "non") : "n/a"}</dd>
        <dt>Source du fallback</dt>
        <dd>${participant?.fallbackSource || "n/a"}</dd>
    ...
    <section class="debug-translation" aria-label="Traduction française">
      <div>
        <h3>Texte original</h3>
        <p>${participant ? participant.text : state.commonQuestion}</p>
      </div>
      <div>
        <h3>Traduction française</h3>
        <p class="translation-text">${participant?.translationFr || "Non renseignée pour ce moment."}</p>
      </div>
    </section>
  `;
}
...
function renderActivitySummary() {
  if (!teacherActivityCard) {
    return;
  }

  if (!stagedScenario.activity && stagedScenario.activitySource === "server") {
    teacherActivityCard.hidden = false;
    activityTitle.textContent = "Activite serveur indisponible";
    activityGoal.innerHTML = `Impossible de recuperer l'activite serveur via ${proto06ServerEndpoint || "/api/activities"}. ${proto06ServerError || hubLaunchState.error || ""} <a href="library-1.1.html">Retour bibliotheque</a>.`;
    if (!hubLoadErrorSent) {
      hubLoadErrorSent = true;
      sendHubRunEvent("error", {
        activityId: hubLaunchState.params.activityId,
        activitySource: stagedScenario.activitySource,
        message: proto06ServerError || hubLaunchState.error || "Activite serveur indisponible"
      });
    }
    return;
  }

  if (!stagedScenario.activity) {
    return;
  }

  teacherActivityCard.hidden = false;
  activityTitle.textContent = stagedScenario.activity.title;
  activityGoal.innerHTML = `${escapeHtml(stagedScenario.activity.pedagogicalGoal)}${aiConfigSummaryHtml()}`;
}
...
async function speakParticipant(participant, index) {
  ...
  const turn = activeAgentRuntime.resolveTurn({
    kind: "participant_testimony",
    participant,
    activity: stagedScenario.activity,
    scenario: stagedScenario.pedagogicalScenario,
    aiConfig: hubLaunchState.aiConfig,
    meetingState: { activeStep, activeParticipantId, currentSequenceIndex }
  });
  lastRuntimeTurn = turn;
  renderDebugPanel();
  addHistory(`${participant.name} - ${participant.role}`, turn.text);
  return speakText(turn.text, turn.speechLang, participant.name, {
...
function handleUserText(text) {
  ...
  sendHubRunEvent("user_answer_submitted", {
    answerLength: cleanText.length,
  ...
  sendHubActivityCompletedOnce("pedagogical_feedback");
}
...
startButton.addEventListener("click", () => {
  ...
  sendHubRunEvent("meeting_started", {
    activityId: stagedScenario.activity?.id || hubLaunchState.params.activityId || "",
    activitySource: stagedScenario.activitySource
  });
  playMeeting();
...
renderDebugPanel();
resolveHubAiConfig();
sendHubRunEvent("proto_loaded", {
  href: window.location.href,
  activityId: hubLaunchState.params.activityId,
  activitySource: hubLaunchState.params.activitySource
});
if (stagedScenario.activity) {
  sendHubRunEvent("activity_loaded", {
    activityId: stagedScenario.activity.id,
    activitySource: stagedScenario.activitySource,
    scenarioId: stagedScenario.activity.scenarioId || "",
    characterCount: Array.isArray(stagedScenario.activity.characterIds)
      ? stagedScenario.activity.characterIds.length
      : stagedScenario.participants.length
  });
}
