...
function aiModeLine(assignment) {
  const config = assignment.aiConfig;
  if (!config) return "";
  const status = config.status === "active" ? "" : ` - ${config.status} / non active`;
  return `<small>Mode : ${config.title}${status}</small>`;
}
          ...
          return `
            <div class="activity-row">
              <span>
                ...
                ${aiModeLine(item)}
                ${debugMeta({ aiConfigId: item.aiConfigId, aiConfig: item.aiConfig })}
              </span>
