...
function launchUrl(proto, assignment) {
  if (assignment.prototypeId === "proto06") {
    const base = proto.launchUrl || "http://127.0.0.1:8788/index-1.1.1.html";
    const params = new URLSearchParams({
      activityId: assignment.activityId,
      activitySource: assignment.activitySource || "server"
    });
    if (assignment.courseId) params.set("courseId", assignment.courseId);
    if (assignment.id) params.set("assignmentId", assignment.id);
    ...
    return `
      <article class="card">
        ...
        ${assignments.map((item) => {
          ...
          const url = launchUrl(proto, item);
          const link = url ? `<a href="${url}" target="_blank" rel="noopener">Lancer</a>` : "<span>Prototype planifie</span>";
          return `
